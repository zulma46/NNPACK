...
#ifdef _WIN32
	const float* input               = (float*)((char*)context->input);
	const float* kernel              = (float*)((char*)context->kernel);
	float* output                    = (float*)((char*)context->output);
#else
...
#ifdef _WIN32
	const float* input                 = (float*)((char*)context->input);
	const uint16_t* kernel             = (uint16_t*)((char*)context->kernel);
	float* output                      = (float*)((char*)context->output);
#else
	const float* input = context->input;
