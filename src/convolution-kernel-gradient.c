...
#ifdef  _WIN32
	const size_t input_channels_subblock_max = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.mr : nnp_hwinfo.cxgemm.mr;
	const size_t output_channels_subblock_max = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.nr : nnp_hwinfo.cxgemm.nr;
#else
...
#ifdef _WIN32
	nnp_fast_tuple_gemm_function fast_gemm_a = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.cX_conjb_transc_only_mr_x_nr : nnp_hwinfo.cxgemm.cX_conjb_transc_only_mr_x_nr;
	nnp_full_tuple_gemm_function full_gemm_a = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.cX_conjb_transc_upto_mr_x_nr : nnp_hwinfo.cxgemm.cX_conjb_transc_upto_mr_x_nr;
	nnp_fast_tuple_gemm_function fast_gemm_b = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.s4cX_conjb_transc_only_mr_x_nr : nnp_hwinfo.cxgemm.s4cX_conjb_transc_only_mr_x_nr;
	nnp_full_tuple_gemm_function full_gemm_b = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.s4cX_conjb_transc_upto_mr_x_nr : nnp_hwinfo.cxgemm.s4cX_conjb_transc_upto_mr_x_nr;
#endif
		...
		for (size_t x = 0; x < output_size.width; x += output_tile.width) {
			...
			for (size_t batch_block_start = 0; batch_block_start < batch_size; batch_block_start += batch_block_max) {
				...
				struct input_transform_context input_transform_context = {
					...
					.row_offset = (uint32_t)doz(input_padding.top, y),
					.column_offset = (uint32_t)doz(input_padding.left, x),
					.row_count = (uint32_t)min(input_size.height - input_y,
						tile_size.height - input_transform_context.row_offset),
					.column_count = (uint32_t)min(input_size.width - input_x,
				...
				struct grad_output_transform_context grad_output_transform_context = {
					...
					.row_count = (uint32_t)min(output_tile.height, output_size.height - y),
					.column_count = (uint32_t)min(output_tile.width, output_size.width - x),
				...
				for (size_t tuple_index = 0; tuple_index < tuple_count; tuple_index += 1) {
					for (size_t input_channels_block_start = 0; input_channels_block_start < input_channels; input_channels_block_start += input_channels_block_max) {
...
#ifdef _WIN32
							.fast_gemm = fast_gemm_a,
							.full_gemm = full_gemm_a
#else
						...
						{
#ifdef _WIN32
							matrix_multiplication_context.fast_gemm = fast_gemm_b;
							matrix_multiplication_context.full_gemm = full_gemm_b;
#else
	...
	struct nnp_profile* profile)
{
	...
	enum nnp_status status = validate_convolution_arguments(
		...
		input_size, input_padding, kernel_size, (struct nnp_size) { .height = 1, .width = 1 },
