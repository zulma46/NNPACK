	...
	uint32_t row_offset, uint32_t column_offset)
{
	...
	union block16x16 block = { 0.0f };

	if (column_count >= simd_width) {
		const psimd_f32 zero = psimd_zero_f32();
		const uint32_t column_end = column_offset + column_count;
		const uint32_t zero_column_start = uint32_t(long long(long long(column_end) & -long long(simd_width)));
		...
		const float* row8 = data + doz(block_size / 2, row_offset) * data_stride;
		float* output = &block.as_float[0][column_offset];
		do {
			const uint32_t column_block = (uint32_t)min(column_count, simd_width);
			row0 += column_block;
			row8 += column_block;
			output += column_block;
		...
		const uint32_t column = (uint32_t)min(column_offset, block_size - simd_width);
		psimd_fft16_real_f32(
