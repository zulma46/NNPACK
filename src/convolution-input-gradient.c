	...
	size_t input_channels_block_size,  size_t batch_subblock_size)
{
	...
	while (input_channels_block_size != 0) {
		...
		full_gemm(
			(uint32_t)batch_subblock_size, (uint32_t)input_channels_subblock_size,
			output_channels_block_size, output_channels_block_start,
			grad_output_transform,
			kernel_transform,
	...
	struct nnp_profile* profile)
{
	...
	const size_t input_channels_subblock_max = fourier_transform ? (bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.nr : nnp_hwinfo.cxgemm.nr) : nnp_hwinfo.sxgemm.nr;
#else
...
#ifdef _WIN32
	float* grad_output_transform = (float*)memory_block;
	float* kernel_transform = (float*)((char*)memory_block + grad_output_transform_size);
	float* grad_input_transform = (float*)((char*)memory_block + grad_output_transform_size + kernel_transform_size);
	nnp_fast_tuple_gemm_function fast_gemm_a = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.s4cX_only_mr_x_nr : nnp_hwinfo.cxgemm.s4cX_only_mr_x_nr;
	nnp_full_tuple_gemm_function full_gemm_a = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.s4cX_upto_mr_x_nr : nnp_hwinfo.cxgemm.s4cX_upto_mr_x_nr;
	nnp_fast_tuple_gemm_function fast_gemm_b = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.cX_only_mr_x_nr : nnp_hwinfo.cxgemm.cX_only_mr_x_nr;
	nnp_full_tuple_gemm_function full_gemm_b = bypass_fft16x16 ? nnp_hwinfo.cxgemm_psimd.cX_upto_mr_x_nr : nnp_hwinfo.cxgemm.cX_upto_mr_x_nr;
#else
					...
					for (size_t batch_block_start = 0; batch_block_start < batch_size; batch_block_start += batch_block_max) {
						...
						if (fourier_transform) {
							...
							if (tuple_index < NNP_COMPLEX_TUPLE_INDEX) {
								matrix_multiplication_context.fast_gemm = fast_gemm_a;
								matrix_multiplication_context.full_gemm = full_gemm_a;
							} else {
